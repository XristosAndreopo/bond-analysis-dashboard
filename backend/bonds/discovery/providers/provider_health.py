...
2. AI Research Provider
   The frontend asks the Django backend to run OpenAI Responses API with
   hosted web search. The backend validates/imports structured JSON and
   calculates missing YTM/duration when enough verified inputs exist.
...
def get_ai_research_provider_status():
    ...
    return {
        ...
        "available": True,
        "mode": "backend_openai_web_search",
        "detail": (
            "AI research is generated by the Django backend through OpenAI "
            "Responses API web search and then validated/imported."
        ),
        "candidate_count": None,
        "file_exists": None,
        "file_name": None,
        "configuration": {
            "backend_openai_client": True,
            "uses_frontend_puter_js": False,
            "backend_discovery_endpoint": "/api/ai-research/discover/",
